...
`IKafkaMessageConsumerFactory.Create` returns a `KafkaMessageConsumer` bound to (topic, group, connection) that runs `_consumer.Consume()` on a dedicated `Task`. Combined with the inbox's `MessageId` deduplication this gives at-least-once delivery semantics.
...
protected override async Task PublishToEventBusAsync(Type eventType, object eventData)
{
    ...
    {
        headers.Add(EventBusConsts.CorrelationIdHeaderName, Encoding.UTF8.GetBytes(CorrelationIdProvider.Get()!));
    }
