...
`RebusDistributedEventBus.PublishToEventBusAsync` copies the current correlation id into Rebus headers and delegates to an internal `PublishAsync`, which either calls the user-supplied `AbpRebusEventBusOptions.Publish` callback or `IBus.Publish(eventData, headers)` after stamping a `Headers.MessageId`:
...
protected virtual async Task PublishAsync(Type eventType, object eventData, Guid? eventId = null, Dictionary<string, string>? headersArguments = null)
{
