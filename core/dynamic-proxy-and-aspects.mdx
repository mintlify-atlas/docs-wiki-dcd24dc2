...
The Castle DynamicProxy bridge (`Volo.Abp.Castle.Core`, which depends only on `Castle.Core` and `Castle.Core.AsyncInterceptor` — there is no Windsor container involved) and the Autofac integration (`Volo.Abp.Autofac`) replay this list during container build, picking the `ITypeList<IAbpInterceptor>` off each `IOnServiceRegistredContext` and wiring up Castle's `ProxyGenerationOptions` with `AbpAsyncDeterminationInterceptor<TInterceptor>` per entry.
