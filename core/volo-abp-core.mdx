...
`AbpApplicationBase.ConfigureServices()` (lines 304–392 in `AbpApplicationBase.cs`) enforces the canonical order:
...
`SetupTelemetryTrackingAsync()` (lines 145–195 of `AbpApplicationBase.cs`) opt-out reports a single activity via `Internal.Telemetry.ITelemetryService.AddActivityAsync(ActivityNameConsts.ApplicationRun)` only when:
