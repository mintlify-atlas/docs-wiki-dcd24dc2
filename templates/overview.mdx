...
│   ├── aspnet-core/        # 7 host variants (MVC, Blazor, Blazor.WebAssembly × {EF, Mongo})
...
├── console/src/            # Single-project console host using AbpApplicationFactory
...
| `app-nolayers` | 7 hosts (`Mvc`, `Mvc.Mongo`, `Blazor.Server`, `Blazor.Server.Mongo`, `Blazor.WebAssembly/Server`, `Blazor.WebAssembly/Server.Mongo`, `Host`, `Host.Mongo`) | none | MVC / Blazor Server / Blazor WASM | Single executable per host; `MyProjectNameModule.cs` depends directly on Account / Identity / OpenIddict modules; `--migrate-database` arg replaces `DbMigrator` |
