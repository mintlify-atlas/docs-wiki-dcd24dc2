...
You can add custom hooks by implementing `IModuleLifecycleContributor` and adding the type to `AbpModuleLifecycleOptions.Contributors`. Inside `Volo.Abp.Modularity.AbpModularityModule` (the framework's bootstrap module) the default list is wired:
