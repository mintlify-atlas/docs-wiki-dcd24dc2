...
The companion managers (`AbpApplicationManager`, `AbpAuthorizationManager`, `AbpScopeManager`, `AbpTokenManager`) extend OpenIddict's `OpenIddictApplicationManager` / `OpenIddictAuthorizationManager` / `OpenIddictScopeManager` / `OpenIddictTokenManager` and add the ABP options/cache wiring. Use them — not the OpenIddict bare managers — when seeding programmatically.
