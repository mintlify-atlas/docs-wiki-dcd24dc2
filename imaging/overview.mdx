...
Because of the **reverse-iteration** rule, the **last module loaded** is asked first. Module load order is controlled by `[DependsOn(...)]` — put the module whose contributors should win **last** in the dependency chain. There is no `Configure<...>` knob for contributor ordering; the `IEnumerable<IImageResizerContributor>` injection follows the container's registration order, and `ImageResizer` / `ImageCompressor` simply iterate that enumeration in reverse.
...
`ResizeImageAttribute.Mode` is a public settable property (not a constructor argument). With no explicit value the field is `default(ImageResizeMode) == ImageResizeMode.None`, which the filter forwards to `new ImageResizeArgs(Width, Height, Mode)`. Because `Mode` is non-nullable on the attribute, the args ctor receives `ImageResizeMode.None` and `ImageResizer.ChangeDefaultResizeMode` does **not** rewrite it from `ImageResizeOptions.DefaultResizeMode` — that sentinel substitution only fires when the args carry `ImageResizeMode.Default`. To pick a mode at the call site use named-argument attribute syntax:

```csharp
[ResizeImage(256, 256, "file", Mode = ImageResizeMode.Crop)]
```

To change the mode for every caller that does pass `ImageResizeMode.Default` (or constructs `ImageResizeArgs` without specifying `mode`, since the args type itself defaults to `ImageResizeMode.Default`), configure it globally:
