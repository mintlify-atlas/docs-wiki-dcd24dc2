...
ABP marks `__AuthenticationMiddlewareSet = true` from inside its own `UseAuthentication` wrapper; if you call `app.UseAuthentication()` directly, the property is also set by ASP.NET Core (the warning checks the same key).
