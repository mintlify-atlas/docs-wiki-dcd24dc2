...
`IRemoteService` (in `Volo.Abp.Http.Abstractions`) is what causes
application services to be auto-exposed as HTTP API endpoints by ABP's
`AspNetCoreMvcConventionalControllerBuilder` — see
[MVC Integration](/aspnetcore/mvc-integration).
