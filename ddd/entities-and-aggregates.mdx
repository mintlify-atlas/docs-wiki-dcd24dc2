...
Both `Entity..ctor()` and `Entity<TKey>..ctor(TKey)` end up running the
parameterless base `Entity()` constructor, which calls
`EntityHelper.TrySetTenantId(this)`. That helper inspects the runtime type for
`IMultiTenant` and, if present, reads the current tenant straight from the
ambient `AsyncLocal` accessor (no DI lookup):
...
public static void TrySetTenantId(IEntity entity)
{
    ...
    ObjectHelper.TrySetProperty(multiTenantEntity, x => x.TenantId, () => tenantId);
}
```
...
sequenceDiagram
    ...
    participant ALC as AsyncLocalCurrentTenantAccessor

    Caller->>Ent: ctor()
    Ent->>Helper: TrySetTenantId(this)
    Helper->>ALC: Instance.Current?.TenantId
    ALC-->>Helper: 11111111-1111-...
    Helper-->>Ent: this.TenantId = ...
```

<Warning>
`AsyncLocalCurrentTenantAccessor.Instance.Current` is the same `AsyncLocal`
that `ICurrentTenant.Change(...)` writes into. Construct multi-tenant entities
inside an active request or a `using (CurrentTenant.Change(...))` scope —
constructing them from a singleton without an ambient tenant scope leaves
`TenantId` at its default (`null`).
</Warning>
