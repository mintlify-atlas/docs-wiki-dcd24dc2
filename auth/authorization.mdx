...
- `PermissionRequirement(Handler)` — single permission. Calls `IPermissionChecker.IsGrantedAsync(context.User, requirement.PermissionName)` and `context.Succeed` on success. Source: `framework/src/Volo.Abp.Authorization.Abstractions/Volo/Abp/Authorization/PermissionRequirement.cs` and `PermissionRequirementHandler.cs`.
- `PermissionsRequirement(Handler)` — batch (used when a single `[Authorize(Permissions = …)]` carries multiple names). Calls the batched `IsGrantedAsync(string[])` overload, returning a `MultiplePermissionGrantResult`.
- `ResourcePermissionRequirement` and `KeyedObjectResourcePermissionRequirementHandler` — same idea for resource permissions, used when the policy name resolves to a resource permission.
