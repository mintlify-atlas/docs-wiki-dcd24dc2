...
Additional dictionary entries whose **key begins with `[o]`** (case-insensitive) are forwarded to the token endpoint by `AddParametersToRequestAsync` as extra `Parameters` on the `ProtocolRequest`. The `[o]` prefix exists so the framework can distinguish protocol parameters from accessor keys without colliding with the strongly-typed properties above; override `AddParametersToRequestAsync` if you want to strip or rewrite the prefix before it hits the wire. Use this hook to push values like `acr_values`, `prompt`, or your own OpenIddict extension parameters through configuration alone.
...
- **Cache key** is `string.Join(",", configuration.Select(x => x.Key + ":" + x.Value)).ToSha256()` (see `IdentityModelTokenCacheItem.CalculateCacheKey`) — every key/value in the `IdentityClientConfiguration` dictionary participates, so switching scope, grant type or any `[o]` parameter yields a different cache entry. Per-tenant uniqueness comes from the tenant-scoped configuration lookup above (and from `AddHeaders` propagating `__tenant`), not from the token cache key itself.
...
`AddParametersToRequestAsync` (called from each `Create…TokenRequestAsync`) walks the `IdentityClientConfiguration` dictionary and forwards every entry whose key starts with `[o]` into the request's `Parameters`. Use that prefix in configuration (`IdentityClients:Default:[o]acr_values`) to propagate custom OAuth parameters. The current-tenant id is propagated separately by `AddHeaders`, which sets the default tenant header (`TenantResolverConsts.DefaultTenantKey` → `__tenant`) on the outbound `HttpClient`.
