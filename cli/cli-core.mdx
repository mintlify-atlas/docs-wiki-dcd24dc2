...
`BundleConfig.Mode` accepts only `None`, `Bundle`, or `BundleAndMinify` (enum `BundlingMode`); the default is `BundleAndMinify`. `Name` defaults to `"global"` and is used as the output filename stem under `wwwroot/`.
